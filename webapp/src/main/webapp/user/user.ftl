...
<@s.form name="userForm" id="userForm" action="${actionValue}" namespace="${namespaceValue}" method="post" title="User Information Form" cssClass="ambra-form" enctype="multipart/form-data">
  ...
  <#--store the email on the page so that it gets set on the action when we go back to save-->
    <input name="email" style="display: none;" value="${email}"/>
    <#--Same for alerts journals-->
    <input name="alertsJournals" style="display: none;" value="${alertsJournals!}"/>

    <#--We need to show the display name if we just submitted a duplicate one-->
    ...
    <#--store the display name on the page so that it gets set on the action when we go back to save-->
      <input name="displayName" style="display: none;" value="${displayName}"/>
    <#else>
